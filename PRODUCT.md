...
1. **Twee taken, één pagina** — bestaande gebruikers moeten direct door kunnen naar hun tool; nieuwe bezoekers moeten het platform kunnen begrijpen vóór ze registreren. De hero bedient de eerste groep, de secties erna de tweede — geen van beide mag de andere blokkeren.
2. **Eén visuele familie** — main (homepage/auth), admin en de tools erachter moeten voelen als hetzelfde platform, niet losse projecten die toevallig samen draaien. Marketing-secties gebruiken hetzelfde systeem (DESIGN.md), niet een los "landingspagina-thema".
...
4. **Nederlandse directheid** — copy is Nederlands, direct, zonder omhaal ("Inloggen", niet "Log hier eenvoudig in!"); ook in de marketing-secties: uitleg in concrete taal, geen marketing-jargon.
5. **Rolbewust zonder drempels** — wat je ziet (apps-grid, adminlink) past bij je rol/capabilities, maar de basisflow (inloggen → bij je tool) blijft voor iedereen identiek simpel.

## Accessibility & Inclusion

WCAG 2.1 AA. Geen specifieke bekende gebruikersbehoeften. Formulieren (login/register/reset-password) moeten volledig met toetsenbord en screenreader te bedienen zijn; foutmeldingen nooit alleen via kleur.
